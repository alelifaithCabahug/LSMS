...
class ResourceController {
	...
	def searchResource(){
		...
		if(name){
			def srchResults = searchableService.search(name)
			def results = srchResults.results
			if(results)
			render(view: "list", model: [resourceInstanceList: results, resourceInstanceTotal: results.size()])	
			else{
				flash.message = message(code: 'Resource not found!')
				redirect(action:"list")
			}
		}else{
