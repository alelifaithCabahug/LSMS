...
class JobController {
    ...
    def show(Long id) {
        ...
        if (!jobInstance) {
            flash.message = message(code: 'default.created message', args: [message(code: 'job.label', default: 'Job'), id])
	...
	def searchJob(){
		...
		if(jobName){
			def srchResults = searchableService.search(jobName)
			def results = srchResults.results
			if(results)
			render(view: "list", model: [jobInstanceList: results, jobInstanceTotal: results.size()])	
			else{
				flash.message = message(code: 'Job is not available!')
				redirect(action:"list")
			}
		}else{
